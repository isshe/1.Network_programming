...
//	 DEBUG;
//	 printf("family = %d\n", family);
	 //居然漏了这句!!! 把AF_INET改为family了
	 sockfd = my_socket(family, SOCK_DGRAM, 0); 
...
/* 以下是第二部分: 处理接口配置 */
     for (ptr = buf; ptr < buf + ifc.ifc_len; )         //注意这里
     {
          ifr = (struct ifreq *) ptr;
		   ...
		   DEBUG;
		   printf("defiened HAVE_SOCKADDR_SA_LEN\n");
           len = max(sizeof(struct sockaddr), ifr->ifr_addr.sa_len);
#else
         switch (ifr->ifr_addr.sa_family) 		//前面ioctl总是返回AF_INET!!!
           {
#ifdef     IPV6
               case AF_INET6:
//               	   DEBUG;
...
/*           DEBUG;
           printf("ifi_name = %s\n", ifr->ifr_name);
           printf("sizeof(ifr->ifr_name) = %lu, len = %d\n",
...
/*漏了这句*/ ifipnext = &ifi->ifi_next;	/* pointer to next one goes here */
           ...
           if (hlen == 0)
           {
                ...
                haddr = ifrcopy.ifr_hwaddr.sa_data;  //地址赋值, 内容并没有
